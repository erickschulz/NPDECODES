...
Eigen::VectorXcd create_mu() {
  ...
  mu_real << -4.1496914847821, 0.192858374584309, 3.56469932115993, -3.18434032487816, 1.27079360432336, 0.398221131357874, -0.0149104581241818, -0.486016878008117, 3.27056201654218, -4.03999102814268, 4.13035390522648, -0.382981058997326, -3.13516563507752;

  Eigen::VectorXd mu_imag(N_dofs);
  mu_imag << 0.638829423213992, 0.0352571144640403, -0.863467224883632, 2.10972704390233, -3.56923020597589, 4.45620403652878, -0.0119650049478216, 0.824035557702947, -1.933150237646, -3.5424244939791, -0.561002581688212, 0.16715551617804, 0.1712965388913;
...
Eigen::SparseMatrix<double> create_D() {
  ...
  D.diagonal() << 0.5, 0.666666666666667, 0.416666666666667, 1.08333333333333, 0.75, 0.416666666666667, 0.666666666666667, 1.25, 1.04166666666667, 0.333333333333333, 0.5, 0.833333333333333, 0.541666666666667;
...
TEST(NonLinSchroedingerEquation, KineticPropagator) {
  ...
  mu1_ref_real << -3.56947555935825, -0.899481130498679, 1.5037668895824, 1.52054950607771, -3.53782254622478, 4.69451086154526, 0.0322136990251863, 0.563187056018217, -1.37681884434939, -2.67718731995605, 3.697139063251, 1.16409150537193, -3.15837046330101;
  Eigen::VectorXd mu1_ref_imag(N_dofs);
  mu1_ref_imag << 3.38175802732124, -0.750151600629607, -2.53402065979762, 2.49681367346699, 0.96057685919604, -2.27253221774087, 0.0475834865954048, -0.883190016287789, -2.79323056125239, 5.15288941038818, -3.28248771823266, -0.0425049970290095, 1.02499143544504;
...
TEST(NonLinSchroedingerEquation, InteractionPropagator) {
  ...
  int N_dofs = mu0.size();
  Eigen::VectorXd mu1_ref_real(N_dofs);
  mu1_ref_real << -2.02038010911912, 0.194070782992831, 1.58498317601217, 3.29216975114174, -3.75919937267198, 4.25397749072451, -0.0149148301353888, 0.356950494994908, -2.80431837652695, 5.3319156923224, 0.95346317103043, -0.348117389186395, 2.77273009303072;
  Eigen::VectorXd mu1_ref_imag(N_dofs);
  mu1_ref_imag << -3.68050358857491, 0.0278199201325495, -3.30764043246613, 1.93716024997555, -0.471954155127836, 1.38564425197975, -0.0119595546282825, 0.887599769147364, -2.56309269992962, 0.664058621646174, 4.05776481089886, 0.231148743205556, -1.47328676597906;
  Eigen::VectorXcd mu1_ref = mu1_ref_real + _i * mu1_ref_imag;
...
TEST(NonLinSchroedingerEquation, SplitStepPropagator) {
  ...
  int N_dofs = mu0.size();
  Eigen::VectorXd mu1_ref_real(N_dofs);
  mu1_ref_real << -3.53454531029302, -0.0845223412116841, -2.28539603933269, -2.92264362555127, 2.20309190220553, -3.0326683108532, 1.24630722905565, -0.118039919478382, 3.63183995368114, -4.71176416835282, 3.84566240825742, 0.386053731999964, -0.441101360403472;
  Eigen::VectorXd mu1_ref_imag(N_dofs);
  mu1_ref_imag << 2.05069831570019, 0.21046236616285, 1.4768731863614, -0.984097910136541, -0.977441622733153, 4.0262428321952, 0.247091542946164, 0.585793525242502, -1.8658312924731, -6.54809413043552, -2.25972859122154, 0.423834193838372, -1.7767992773605;
  Eigen::VectorXcd mu1_ref = mu1_ref_real + _i * mu1_ref_imag;
