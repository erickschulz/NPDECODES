...
double solveTransport(const coord_t& x,
                      int K, double t,
                      FUNCTOR_V&& v, FUNCTOR_U0&& u0)
{
...
// Auxiliary function of 'semiLagrangeSource'
double evalFEfunction(const coord_t&  x,
                      const vector_t& u)
{
    ...
    for(int i=0; i<u_all.size(); ++i) {
        if((i>M) && ((i+1) % (M+1) != 0) && (i < M*(M+1)) && (i % (M+1) != 0)) {
            u_all(i) = u(cnt);
            ++cnt;
        }
    }

    Eigen::Vector2i x_idx; x_idx << std::floor(x(0)/h), std::floor(x(1)/h);
    ...
    Eigen::Vector4i indices; indices << ii, ii+1, ii+M+2, ii+M+1;
    ...
    coord_t x_loc; x_loc << std::fmod(x(0), h), std::fmod(x(1), h);
...
vector_t semiLagrangeSource(const vector_t& u_old,
                            double tau, double t,
                            FUNCTOR&& velocity)
{
    ...
    return f / (M*M); // * 1 (from $[0,1]^2$) * 4 (from no. of adjacent squares) / 4 (from no. of vertices of square)
}
/* SAM_LISTING_END_2 */
...
/*
    ...
    for(int i=0; i<N; ++i) {
        mat.insert(i,i) = 1. / (M*M); // * 1 (from $[0,1]^2$) * 4 (from no. of adjacent squares) / 4 (from no. of vertices of square)
    }
*/
    ...
    auto velocity = [](double t, const coord_t& x) { return ( coord_t() << -(x(1)-0.5), x(0)-0.5 ).finished(); };
...
int main()
{
    ...
    auto v  = [](double t, const coord_t& x) { return ( coord_t() << -x(1), x(0) ).finished(); };
