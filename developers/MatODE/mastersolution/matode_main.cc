...
int main() {
  ...
  std::cout << std::setw(sep[0]) << "step" << std::setw(sep[1]) << "exp. Eul"
            << std::setw(sep[2]) << "imp. Eul" << std::setw(sep[3]) << "IMP"
            << std::endl;
  std::cout << std::setw(sep[0]) << "-1" << std::setw(sep[1])
            << (Mexpeul.transpose() * Mexpeul - I).norm() << std::setw(sep[2])
            << (Mimpeul.transpose() * Mimpeul - I).norm() << std::setw(sep[3])
  ...
  for (unsigned int j = 0; j < 20; ++j) {
    ...
    std::cout << std::setw(sep[0]) << j << std::setw(sep[1])
              << (Mexpeul.transpose() * Mexpeul - I).norm() << std::setw(sep[2])
              << (Mimpeul.transpose() * Mimpeul - I).norm() << std::setw(sep[3])
              << (Mimp.transpose() * Mimp - I).norm() << std::endl;
  }
