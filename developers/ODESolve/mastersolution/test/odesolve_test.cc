...
TEST(ODESolve, odeintequi) {
  ...
  reference << 1.0 ,1.125, 1.265625, 1.423828125, 1.601806640625, 1.80203247070312, 2.02728652954102, 2.28069734573364, 2.56578451395035;
...
TEST(ODESolve, odeintssctrl) {
  ...
  y0 << 1.0;
  double T = 1.0;
	std::vector<Eigen::VectorXd> vector = odeintssctrl(Psi, T, y0, 0.01, 1, 10e-5, 10e-5, 10e-5);
