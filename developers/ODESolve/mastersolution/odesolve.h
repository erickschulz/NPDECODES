...
//! \param[in] Psi original evolution operator, must have operator(double, const Eigen::VectorXd&)
...
Eigen::VectorXd psitilde(const DiscEvlOp& Psi, unsigned int p, double h, const Eigen::VectorXd & y0) {
#if SOLUTION
...
/* SAM_LISTING_BEGIN_1 */
template <class DiscEvlOp>
std::vector<Eigen::VectorXd> odeintequi(const DiscEvlOp& Psi, double T, const Eigen::VectorXd &y0, int N) {
    double h = T / N;
    double t = 0.;
...
/* SAM_LISTING_BEGIN_3 */
template <class DiscEvlOp>
std::vector<Eigen::VectorXd> odeintssctrl(const DiscEvlOp& Psi, unsigned int p, const Eigen::VectorXd &y0,
    ...
    while( t < T && h > hmin ) {
        ...
        if( est < tol ) {
#else // TEMPLATE
        Eigen::VectorXd y_high = Eigen::VectorXd::Zero(y0.size()); // TODO: fix this line
        Eigen::VectorXd y_low = Eigen::VectorXd::Zero(y0.size()); // TODO: fix this line
        double est = (y_high - y_low).norm();
        // TODO: update $h$
