...
void errors(const Function &f, const double &T, const Eigen::VectorXd &y0, const MatrixXd &A, const Eigen::VectorXd &b) {
    ...
    for(int k = 0; k < 15; k++) {
        ...
        if (k>0 && test) {
            ...
            std::cout << left << std::setw(10) << std::setfill(' ') << "Approximated order = " << order[k-1] << std::endl;
            sum += order[k-1];
            count = k;
        }
        else std::cout << std::endl;
    }
    std::cout << "Average approximated order = " << sum / count << std::endl << std::endl;
}
