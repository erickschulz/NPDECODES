...
TEST(SDIRKMethodOfLines, assembleGalerkinMatrices) {
  ...
  const lf::io::GmshReader reader(std::move(mesh_factory), CURRENT_SOURCE_DIR "/../../meshes/simple.msh");
  ...
  std::pair<Eigen::SparseMatrix<double>, Eigen::SparseMatrix<double>> pair = assembleGalerkinMatrices(dofh, c);
  ...
  A_ref << 1.66667, 0.166667, 0, 0.166667, -1,
  		   0.166667, 1.66667, 0.166667, 0, -1,
		   0, 0.166667, 1.66667, 0.166667, -1,
		   0.166667, 0, 0.166667, 1.66667, -1,
		   -1, -1, -1, -1, 4;

  Eigen::MatrixXd M_ref(N_dofs, N_dofs);

  M_ref << 0.0833333, 0.0208333, 0, 0.0208333, 0.0416667,
  		   0.0208333, 0.0833333, 0.0208333, 0, 0.0416667, 
		   0, 0.0208333, 0.0833333, 0.0208333, 0.0416667,
		   0.0208333, 0, 0.0208333, 0.0833333, 0.0416667,
		   0.0416667, 0.0416667, 0.0416667, 0.0416667, 0.16667;
...
TEST(SDIRKMethodOfLines, solveTemperatureEvolution) {
  ...
  std::pair<Eigen::VectorXd, Eigen::VectorXd> pair = solveTemperatureEvolution(dofh, m, c, init);
  ...
  eng_ref << 5, 3.68005, 2.73267, 2.02427, 1.50051, 1.11206, 0.824213, 
  			 0.610865, 0.452744, 0.335552, 0.248695, 0.18432, 0.136609;
