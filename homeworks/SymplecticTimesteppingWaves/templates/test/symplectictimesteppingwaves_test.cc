...
TEST(SymplecticTimesteppingWaves, assembleGalerkinMatrix) {
  ...
  Eigen::SparseMatrix<double> galMat = assembleGalerkinMatrix(fe_space, one, one, one);
  
  Eigen::MatrixXd sol(galMat);

  Eigen::MatrixXd ref_sol(5,5);
  ref_sol << 1.75, 0.1875, 0, 0.1875, -0.958333, 
  			 0.1875, 1.75, 0.1875, 0, -0.958333, 
			 0, 0.1875, 1.75, 0.1875, -0.958333, 
			 0.1875, 0, 0.1875, 1.75, -0.958333, 
			 -0.958333, -0.958333, -0.958333, -0.958333, 4.16667;
...
TEST(SymplecticTimesteppingWaves, solvewave) {
  ...
  std::pair<Eigen::VectorXd, Eigen::VectorXd> sol = solvewave(fe_space, c, u0, v0, T, m);
  ...
  ASSERT_NEAR((sol.second - ref_sol_second).lpNorm<Eigen::Infinity>(), 0.0, tol);
