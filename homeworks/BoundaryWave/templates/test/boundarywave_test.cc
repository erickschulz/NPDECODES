...
TEST(BoundaryWave, buildM) {
    ...
    reference_M <<0.666667,0.166667       ,0,0.166667       ,0,
        0.166667,0.666667,0.166667       ,0       ,0,
        0,0.166667,0.666667,0.166667       ,0,
        0.166667       ,0,0.166667,0.666667       ,0,
        0       ,0       ,0       ,0       ,0;

    for (int i =0; i < 5; i++){
        for (int j =0; j <5; j++){
            ASSERT_NEAR(reference_M(i,j), M(i,j),eps);
        }
    }   
...
TEST(BoundaryWave, buildA) {
    ...
    reference_A << 1.33333       ,0       ,0       ,0 ,-1.33333,
           0  ,1.66667       ,0       ,0 ,-1.66667,
           0       ,0        ,2       ,0       ,-2,
           0       ,0       ,0  ,1.66667 ,-1.66667,
        -1.33333 ,-1.66667       ,-2 ,-1.66667  ,6.66667;

    for (int i = 0; i < 5; i++){
        for (int j =0; j <5; j++){
            ASSERT_NEAR(reference_A(i,j), A(i,j),eps);
        }
    }   
}
...
TEST(BoundaryWave, solveBoundaryWave) {
    ...
    double eps = 1.0e-5;
    ASSERT_TRUE(discrete_solution.size()==reference_solution.size());
    for (int i = 0; i<5; i++){
        ASSERT_NEAR(reference_solution(i), discrete_solution(i),eps);
    }
}
}
