...
TriaMesh2D::TriaMesh2D(const string &filename) {
  ...
  auto mesh_path = here.parent_path() / filename;
  string filepath = mesh_path.string();
  ifstream mesh_file(filepath, ifstream::in);
  cout << "Loading mesh from file " << filepath << endl;
  if (!mesh_file.good()) {
